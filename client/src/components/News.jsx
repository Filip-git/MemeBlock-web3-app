...
export const News = () => {
    ...
    const fetchNews = async () => {
        try {
            ...
            const newArticles = response.data.data.filter(
                (article) => article.title && article.description && article.language === 'en' && article.image_url
            );
            ...
            if (newArticles.length === 0) {
                ...
                const newBatch = newArticles.slice(0, batchSize);
                setNewsData((prevNewsData) => [...prevNewsData, ...newBatch]);
            }
        } catch (error) {
